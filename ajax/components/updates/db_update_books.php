<?php
// Updates the desciption, price, categoryId, previewLink, displayImage of a book through the `books` table in database `bookstore`. Takes in input the desciption, price, categoryId, previewLink, displayImage to be updated and outputs a JSON with response code and update id.
...
$sql = "UPDATE `books` SET `description`=?,`price`=?,`categoryId`=?,`previewLink`=?, `displayImage`=? WHERE `isbn`=? and `title`=? and `publicationDate`=? and `edition`=? and `publisherId`=?";
...
$publicationDate = number_format($update_input_query->publicationDate);
...
// REMOVE - TO DO 🔲
ChromePhp::log("Updated book: book_isbn=$update_id");
